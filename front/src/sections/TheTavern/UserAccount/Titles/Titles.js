...
const Titles = ({ show, defaultTitles, updateTitle }) => {
  ...
  const listItems = defaultTitles.map(title => <Title key={title} show={show} selectTitle={() => clickHandler(title)} name={title} />)
  ...
  return (
    <ul ref={list} className={styles.TitleList}>{listItems}</ul>
  );
};
